...
public class Argal {
	...
	public void load(String name_schema, String name_table, int year, String path, String fields) throws SQLException {
		...
		s = "delete from " + name_schema + "." + name_table + " where sikoum regexp 'סיכום'";
	...
	public void insertTo101(String name_schema, String name_table_101, String name_table, int cid) throws SQLException {
		...
		s = "insert ignore into " + name_schema + "." + name_table_101
				...
				+ "\n select " + cid
				+ ", dyear, concat(last_name, ' ' , first_name), id, symbol, concat(symbolName, ' - ', type) , m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, type, 'original_argal'  , id  "
		...
		//bruto
		s = "update " + name_schema + "." + name_table_101 + "  set type = ';b_tlush;', symbol = 2500 where symbolname regexp 'תשלומים - סיכומים' or symbolname regexp 'שווה כסף - סיכומים';";
		tr.Insertintodb1(s);
		...
		s = "update " + name_schema + "." + name_table_101 + "  set type = ';;' where symbol = 200 and  type regexp 'תשלומים' ;";
		tr.Insertintodb1(s);
		...
		//pensya
		s = "update " + name_schema + "." + name_table_101 + "  set type_for_gemel = ';shulam_maavid;pensya;sum_shulam;' where type regexp 'חובה' ;";
		tr.Insertintodb1(s);
		
		

		s = "update " + name_schema + "." + name_table_101 + "  set type = ';nikuy_chova;semelbb;', type_for_gemel = '' where symbolname regexp 'מס בריאות - נכויי חובה' ;";
		tr.Insertintodb1(s);

		s = "update " + name_schema + "." + name_table_101 + "  set type = ';nikuy_chova;semelbilo;', type_for_gemel = ''  where symbolname regexp 'ביטוח לאומי - נכויי חובה' ;";
		tr.Insertintodb1(s);

		s = "update " + name_schema + "." + name_table_101 + "  set type = ';nikuy_chova;semelmh;', type_for_gemel = ''  where symbolname regexp 'מס הכנסה - נכויי חובה' ;";
		tr.Insertintodb1(s);

		s = "update " + name_schema + "." + name_table_101 + " set  type_for_gemel = '' where symbolname regexp 'היטל עובדים זרים - השתתפות' ;";
		tr.Insertintodb1(s);
		

		s = "update " + name_schema + "." + name_table_101 + "  set type = ';tlush_reshut;' where type regexp 'נכויי רשות';";
		tr.Insertintodb1(s);
		...
		//bituah leumi
		s = "update " + name_schema + "." + name_table_101 + "  set type = ';semelbilm;', type_for_gemel = '' where symbolname regexp 'ביטוח לאומי - השתתפות';";
		tr.Insertintodb1(s);
	...
	void load_alphon(String name_schema, String string, String path, int cid) throws SQLException {
		...
		String a = "load data local infile '" + path + "/alphon.csv' "
				...
				"FIELDS TERMINATED BY ','  ENCLOSED BY  '\"' LINES TERMINATED BY '\n'  IGNORE 1 LINES "
				+ "(sikoum,  num_worker,  last_name, first_name, "

//                + "mahlaka, "
                + "tat, "
+ "is_male, id, marital_status, birthday ) set cid = " + cid + " ;";
